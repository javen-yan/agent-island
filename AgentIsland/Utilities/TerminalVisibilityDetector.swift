...
struct TerminalVisibilityDetector {
    ...
    /// Check if an agent session is currently focused (user is looking at it)
    /// - Parameter sessionPid: The PID of the agent process
    ...
    static func isSessionFocused(sessionPid: Int) async -> Bool {
        ...
        let resolvedBackend = await TerminalMultiplexerRegistry.shared.resolvedBackend(
            pid: sessionPid,
            tty: nil,
        ...
        if let resolvedBackend {
            let adapter = await TerminalMultiplexerRegistry.shared.adapter(for: resolvedBackend)
