...
struct ProcessTreeBuilder: Sendable {
    ...
    /// Whether there are still processes attached to a specific TTY.
    /// Useful for deciding whether a terminal conversation still exists even
    /// after a bridge or hook reports an ended state.
    nonisolated func hasProcesses(onTTY tty: String, tree: [Int: ProcessInfo]) -> Bool {
        tree.values.contains { info in
            info.tty == tty
        }
    }
}
