...
struct NotchView: View {
    ...
    private func handlePendingSessionsChange(_ sessions: [SessionListState]) {
        ...
        let newlyInteractiveSessions = interactiveSessions.filter { newInteractiveIds.contains($0.stableId) }

        if !newlyInteractiveSessions.isEmpty {
            triggerAttentionFeedback(for: newlyInteractiveSessions)
        }

        if let target = newlyInteractiveSessions
        ...
        if !newPendingIds.isEmpty &&
           newInteractiveIds.isEmpty &&
           viewModel.status == .closed &&
           !TerminalVisibilityDetector.isTerminalVisibleOnCurrentSpace() {
            viewModel.notchOpen(reason: .notification)
        }

        previousInteractiveAttentionIds = currentInteractiveIds
    ...
    private func triggerAttentionFeedback(for sessions: [SessionListState]) {
    ...
    private func shouldPlayNotificationSound(for sessions: [SessionListState]) async -> Bool {
