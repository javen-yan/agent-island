    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            if let bgId = result.backgroundTaskId {
                HStack(spacing: 4) {
                    Image(agentIcon: "clock.arrow.circlepath")
                        .font(.system(size: 10))
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ForEach(Array(result.newTodos.enumerated()), id: \.offset) { _, todo in
                HStack(spacing: 6) {
                    // Status icon
                    Image(agentIcon: todoIcon(for: todo.status))
                        .font(.system(size: 10))
            ...
            ForEach(Array(result.questions.enumerated()), id: \.offset) { index, question in
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    if let answer = result.answers["\(index)"] {
                        HStack(spacing: 4) {
                    Image(agentIcon: "arrow.turn.down.right")
                                .font(.system(size: 9))
                            Text(answer)
                                .font(.system(size: 11, weight: .medium))
                        }
